...
const isCharInvisibleWidth = (codePoint) => (
  codePoint <= 0x1f ||
  (codePoint >= 0x7f && codePoint <= 0x9f) ||
  (codePoint >= 0x300 && codePoint <= 0x36f)
)
const isCharFullWidth = (codePoint) => (
...
const hasLocaleVersion = (page, localeCode) => Object.hasOwn(page?.slugs || {}, localeCode)
...
const computeBuffer = () => {
  if (globalThis.window) return 100
  const rootStyles = getComputedStyle(document.documentElement)
  const offset = Number.parseFloat(rootStyles.getPropertyValue('--header-offset')) || 90
  return offset
}
...
const runAfterFrame = (cb) => {
  ...
  if (globalThis.window.requestAnimationFrame === 'function') {
      ...
      <nav class="px-3 lg:px-6 py-3 lg:py-4 flex justify-between items-center">
        <div>
          <a :href="baseHomePath" aria-label="Home" @click.prevent="handleHomeClick">
            <img class="rounded-[50%] w-7 lg:w-12.5 inline-block" alt="WalksCloud Logo" src="/logo-gradual.svg">
        ...
        <div v-if="!isNotFound" class="space-x-4 hidden lg:flex">
          <a v-bind:key="key" v-for="(name, key) in $tm('nav-links')" :class="{ 'text-primary': activeSection === key, 'dark:text-white': activeSection !== key }" class="hover:text-primary"
            :href="linkHref(key)" @click.prevent="handleSectionClick($event, key)">{{ name }}</a>

          <div class="ms-2 locale-changer inline-block">
            <ULocaleSelect class="wc-locale-select w-auto" :model-value="currentLocale" :locales="localeSelectOptions" size="sm" variant="ghost" :search-input="false" :ui="{
              base: 'w-auto min-w-0 pl-2.5 pr-2.5',
              content: 'z-[70]',
              value: 'truncate-none whitespace-nowrap',
              item: 'wc-locale-item',
              itemLabel: 'whitespace-nowrap pr-0',
            }" :style="{ width: localeSelectWidth }" @update:model-value="handleDesktopLocaleChange" />
          </div>
        </div>
      </nav>
    </div>

    <UBanner v-if="localeBanner" color="warning" icon="i-lucide-languages" :ui="{
      ...
      <template #title>
        <span class="flex flex-col leading-4 whitespace-normal wrap-break-word sm:block">
          <span class="font-semibold block sm:inline">{{ localeBanner.title }}</span>
          <span class="font-normal block wrap-break-word sm:inline sm:ml-2">{{ localeBanner.message }}</span>
        </span>
      </template>
    </UBanner>
    ...
    <div v-if="isOpen && !isNotFound" class="lg:hidden bg-white/85 dark:bg-[#222831]/85 py-6 text-center flex flex-col space-y-4">
      ...
      <div class="mt-2 locale-changer">
        <div class="inline-flex rounded-lg overflow-hidden border border-gray-300 dark:border-gray-600">
          <button v-for="locale in localeOptions" :key="`locale-${locale}`" :class="[$i18n.locale === locale ? 'bg-primary text-white' : 'bg-white text-gray-700 dark:bg-[#222831]/85 dark:text-white']"
            @click="setLocaleDirect(locale)" class="px-4 py-2 text-sm">
            {{ $t(`locales.${locale}`) }}
          </button>
        </div>
      </div>
