...
- Normal image files are loaded as the bundled asset URL selected by the browser.
- Mermaid diagrams are opened as inline SVG in the viewer so vector rendering is preserved instead of rasterizing the diagram.
- Canvas media is exported as an in-memory canvas blob using the current device pixel ratio up to `config/media-viewer.yaml` `canvas.maximumDevicePixelRatio`.
- No physical `@2x` or `@3x` image files are generated by the viewer.
- Viewer zoom limits are controlled by `config/media-viewer.yaml` `zoom.minimumScale` and `zoom.maximumScale`.
